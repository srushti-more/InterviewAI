...
export default function Feedback() {
  ...
  if (!analysisData) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-50">
        <h2 className="text-2xl font-bold text-slate-300 mb-6">No Analytics Data Found</h2>
  ...
  return (
    <div className="min-h-screen flex font-sans bg-slate-950 text-slate-50 selection:bg-indigo-500/30">
      ...
      {/* Sidebar Navigation */}
      <div className="w-64 bg-slate-900 border-r border-slate-800 p-6 hidden lg:flex flex-col fixed h-full z-10">
      ...
      <div className="flex-1 p-6 lg:p-10 overflow-y-auto lg:ml-64">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-slate-800 pb-6 gap-4">
            <div>
              <h2 className="text-3xl font-semibold text-slate-50 tracking-tight">Performance Analytics</h2>
              <p className="text-slate-400 mt-2">Session logged on {new Date(analysisData.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
            </div>
            <button onClick={() => navigate('/dashboard')} className="bg-slate-900 border border-slate-700 hover:border-indigo-500 text-slate-300 font-semibold py-2.5 px-6 rounded-xl transition-all shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* Score Ring */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl p-8 flex flex-col items-center justify-center relative">
              <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-6">Overall Rating</h3>
              <div className="relative w-40 h-40 flex items-center justify-center">
                <svg className="transform -rotate-90 w-full h-full drop-shadow-md" viewBox="0 0 140 140">
                  <circle cx="70" cy="70" r={radius} stroke="currentColor" strokeWidth="10" fill="transparent" className="text-slate-800" />
                  <circle 
                ...
                <div className="absolute flex flex-col items-center justify-center mt-1">
                  <span className="text-5xl font-bold text-slate-50 tracking-tighter">{analysisData.score || 0}</span>
                </div>
              </div>
              
              {!loadingHistory && previousInterview && (
                <div className={`mt-6 flex items-center text-sm font-semibold px-3 py-1 rounded-full border ${progressDifference >= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                  {progressDifference >= 0 ? (
                    <svg className="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg>
                  ) : (
                    <svg className="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path></svg>
                  )}
            ...
            {/* AI Summary */}
            <div className="md:col-span-2 bg-slate-900 rounded-2xl shadow-xl border border-slate-800 flex flex-col overflow-hidden relative">
              <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent"></div>
              <div className="p-6 border-b border-slate-800/50 bg-slate-900/50 flex items-center">
                <svg className="w-5 h-5 mr-3 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">Executive Summary</h3>
              </div>
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                ...
                {analysisData.compliment && (
                  <div className="bg-cyan-500/5 border border-cyan-500/10 rounded-xl p-4 flex items-start mt-4">
                    <span className="text-xl mr-3" role="img" aria-label="lightbulb">⚡</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-xl relative overflow-hidden">
              ...
              <h4 className="font-semibold text-slate-200 mb-5 flex items-center">
              ...
              <ul className="space-y-3.5">
                {analysisData.strengths && analysisData.strengths.map((str, i) => (
                  <li key={i} className="text-sm text-slate-400 flex items-start leading-relaxed">
                    <span className="mr-3 mt-1 text-emerald-400 text-[10px]">■</span> {str}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-xl relative overflow-hidden">
              ...
              <ul className="space-y-3.5">
                {analysisData.weaknesses && analysisData.weaknesses.map((weak, i) => (
                  <li key={i} className="text-sm text-slate-400 flex items-start leading-relaxed">
                    <span className="mr-3 mt-1 text-red-400 text-[10px]">■</span> {weak}
                  </li>
                ))}
              </ul>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {analysisData.repeatedWords && analysisData.repeatedWords.length > 0 ? (
                    analysisData.repeatedWords.map((word, i) => (
                      <span key={i} className="bg-slate-800 border border-slate-700 text-amber-400 px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider">
                        "{word}"
                      </span>
                    ))
                  ) : (
                    <span className="text-sm text-slate-500">No major filler words detected.</span>
                  )}
                </div>
              </div>
                ...
                <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-xl relative overflow-hidden">
                  ...
                  <ul className="space-y-3.5">
                    {analysisData.suggestions.map((sug, i) => (
                      <li key={i} className="text-sm text-slate-400 flex items-start leading-relaxed">
                        <span className="mr-3 mt-1 text-indigo-400 font-bold">→</span> {sug}
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="bg-slate-900 rounded-2xl shadow-xl border border-slate-800 overflow-hidden mt-8">
            <div className="bg-slate-900/80 border-b border-slate-800 p-6">
               <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">Session Transcript</h3>
            </div>
            <div className="p-0 divide-y divide-slate-800/50">
              {analysisData.qaPairs && analysisData.qaPairs.map((pair, index) => (
                <div key={index} className="p-6 md:p-8 hover:bg-slate-800/30 transition-colors flex flex-col md:flex-row gap-6">
                  <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 text-indigo-400 flex items-center justify-center font-bold flex-shrink-0 shadow-sm">
                    Q{index + 1}
                  </div>
                  
                  <div className="flex-1 space-y-5">
                    <div>
                      <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">Interviewer System</span>
                      <p className="text-slate-200 font-medium text-base">{pair.question}</p>
                    </div>
                    
                    <div className="bg-slate-950/50 rounded-xl p-5 border border-slate-800/80 relative">
                      <span className="text-[11px] font-bold text-cyan-500 uppercase tracking-wider mb-2 flex items-center">
                         Candidate Input
                      </span>
                      <p className="text-slate-400 text-sm leading-relaxed whitespace-pre-wrap">{pair.answer}</p>
                    </div>
                  </div>
                </div>
              ))}
