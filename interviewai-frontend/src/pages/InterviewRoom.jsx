...
export default function InterviewRoom() {
  ...
  const [hasStarted, setHasStarted] = useState(false); // Handles browser autoplay block and sliding chat UI
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          // REAL BACKEND CALL - Send Audio to Server
          ...
          if (response.ok) {
            ...
            if (questionCount >= MAX_QUESTIONS) {
              ...
              playAiVoice(closingMessage, () => {
                handleEndInterview(); // Triggers the analysis flow and navigation
              });
            } else {
  ...
  const handleEndInterview = async () => {
    ...
    try {
      // REAL BACKEND CALL - Final Analysis
      ...
      if (response.ok) {
        // Navigate to feedback page AND pass the data object we just generated
  ...
  return (
    /* Apply background image to the main wrapper */
    <div 
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-grow relative">
        ...
        {/* Right Column: Sliding Live Transcript Overlay */}
        <div 
